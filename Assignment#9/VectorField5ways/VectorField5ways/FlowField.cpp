...
// Set all the forces in the field to point at the last mouse position
void FlowField::setMousePos() {
	for (int y = 0; y<flowList.size(); y++) {
		for (int x = 0; x<flowList[y].size(); x++) {
			ofVec2f mousePoint = ofVec2f(ofGetMouseX(), ofGetMouseY());
			ofVec2f tempVec = mousePoint - ofVec2f(x*resolution, y*resolution);
			flowList[y][x].set(tempVec);
		}
	}
	
}

//Set all the forces in the field to point to the center 
void FlowField::setCenter() {
	for (int y = 0; y<flowList.size(); y++) {
		for (int x = 0; x<flowList[y].size(); x++) {
			ofVec2f tempVec = ofVec2f(ofGetWidth()/2, ofGetHeight()/2) - ofVec2f(x*resolution, y*resolution);
			flowList[y][x].set(tempVec);
		}
	}
...
// Set everything to point in the same direction again 
void FlowField::reset() {
	for (int y = 0; y<flowList.size(); y++) {
		for (int x = 0; x<flowList[y].size(); x++) {
			flowList[y][x].set(ofVec2f(1,0));
		}
	}
